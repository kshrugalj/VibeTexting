...
def main():
    ...
    parser.add_argument("--model", default="llama3", help="Model id to use for the selected backend (default: llama3)")
    parser.add_argument("--backend", choices=["auto", "ollama", "lmstudio"], default="auto", help="Local inference backend to use")
    ...
    while True:
        ...
        print(f"\nGenerating a reply using local backend '{args.backend}' ({args.model})...")
        prompt = build_prompt(
            original,
            vibe_content,
            chat_history,
