...
def call_lmstudio(prompt: str, model: str) -> str:
    ...
    try:
        ...
        if "timed out" in err_text or "timeout" in err_text:
            return (
                "Error connecting to LM Studio: timed out while waiting for model output.\n"
                "Try increasing timeout with VIBETEXT_LMSTUDIO_TIMEOUT_SECONDS (for example 600)."
            )
        return (
...
def call_local_llm(prompt: str, model: str = "llama3", backend: str = "auto") -> str:
    ...
    # auto mode: prefer LM Studio for Gemma-family models, otherwise Ollama
    lowered_model = (model or "").lower()
    if "gemma" in lowered_model:
        lmstudio_reply = call_lmstudio(prompt, model)
        if not lmstudio_reply.startswith("Error"):
            return lmstudio_reply
        ollama_reply = call_ollama(prompt, model)
        if not ollama_reply.startswith("Error"):
            return ollama_reply
        return f"{lmstudio_reply}\n\n{ollama_reply}"
